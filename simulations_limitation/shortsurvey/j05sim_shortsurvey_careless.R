...
n              <- 500L
scale_size     <- 10L
num_dropped    <- 4L # number of traits dropped
num_scales     <- 6L * (5L - num_dropped)
num_likert     <- 5L
batch_size     <- 10L
scale_sizes    <- rep(scale_size, num_scales) # for the option 'factors' in reliability benchmark method
...
dir_save <- "simulations_limitation/shortsurvey/results"
...
careless_start <- carelessness_start_fn(n = n, type = suffix_location, num_items = num_items)
...
for(a in seq_along(prop_crls_arr))
{
  ...
  antonym_mat <- reliability_mat <- matrix(NA_real_, n, R)
  ...
  for(r in seq_len(R)){
    ...
    # randomly sample the traits that are about to be dropped
    traits_dropped <- sample(c("A", "E", "N", "O", "C"), replace = FALSE, size = num_dropped)
    data_drop <- johnson2005_droptrait(data = data, traits_drop = traits_dropped)
    data <- data_drop$data
    ...
    data_posword <- recode(data_contam, keys = data_ls$keys[data_drop$idx_kept], xmax = 5L, xmin = 1L)
    
    ## ... and to be ordered by facets
    data_posword <- data_posword[,data_drop$item_order_ordrerd]
  ...
  save(out_CP_LS_SC, out_CP_LS, out_CP_SC,
       ...
       prop_crls, alpha, n, num_items, mc_cores, reliability_mat, antonym_mat, num_dropped,
       file = paste0(dir_save, "/", suffix_type, "_num-traits-dropped=", num_dropped,  "_", suffix_location, "_prop-crls=", prop_crls, "_n=", n,  "_R=", R, ".Rdata"))
