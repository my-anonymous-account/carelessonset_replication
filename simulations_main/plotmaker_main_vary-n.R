...
load(paste0(dir_load, "/gatheredresults_CODERS_", 
       suffix_type, "_varysamplesize", "_R=", R, ".Rdata"))
...
P_crls_fnr <- 
  ...
  geom_line() +
  ylim(0,1) +
  facet_grid(rows = vars(type), cols = vars(samplesize), scales = "fixed", labeller = label_parsed) +
...
ggsave(filename = paste0("plot-subgroups_", suffix_type, "_vary-n", "_R=", R, "_fnr.pdf"),
       plot = P_crls_fnr,
       path = dir_save,
       device = "pdf",
       width = 7.5,
...
ggsave(filename = paste0("plot-subgroups_", suffix_type, "_vary-n", "_R=", R, "_mae.pdf"),
       plot = P_crls_mae,
       path = dir_save,
       device = "pdf",
       width = 7.5,
...
P_crls <- 
  ggplot(data = df_careless, 
         mapping = aes(x = prop_crls, y = value, color = method, linetype = sig_level, alpha = sig_level)) +
  geom_line() +
  facet_grid(rows = vars(measure), cols = vars(samplesize), scales = "free_y", labeller = label_parsed, switch = "y") +
  labs(y = NULL) + 
  scale_linetype_manual("Significance\nlevel", values = c("dashed", "solid", "twodash")) +
  scale_alpha_manual("Significance\nlevel", values = c(0.4, 1, 0.4)) + 
  scale_color_manual("CODERS", values = c("#619CFF", "#F8766D", "black")) +
...
ggsave(filename = paste0("plot-careless_", suffix_type, "_vary-n", "_R=", R, ".pdf"),
       plot = P_crls,
       path = dir_save,
       device = "pdf",
       width = 7.5,
...
P_att <- 
  ...
  geom_line() +
  ylim(0,1) + 
  facet_grid(cols = vars(samplesize), labeller = label_parsed) +
  scale_linetype_manual("Significance\nlevel", values = c("dashed", "solid", "twodash")) +
  scale_alpha_manual("Significance\nlevel", values = c(0.4, 1, 0.4)) + 
...
ggsave(filename = paste0("plot-attentive_", suffix_type, "_vary-n", "_R=", R, ".pdf"),
       plot = P_att,
       path = dir_save,
       device = "pdf",
       width = 7.5,
