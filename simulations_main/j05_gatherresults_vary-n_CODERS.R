...
colnames(RESULTS) <- c("MAE", "FNR", "type", "FPR", "method", "prop_crls", "sig_level", "samplesize")

# convert to long format
RESULTS_long <- reshape2::melt(data = RESULTS, 
                              id.vars = c("method", "type", "prop_crls", "samplesize", "sig_level"), 
...
RESULTS_out$samplesize <-
  paste0('n*" = "*', as.character(RESULTS_long$samplesize)) # to-be-parsed labels
RESULTS_out$samplesize <- factor(RESULTS_out$samplesize, levels = unique(RESULTS_out$samplesize))
