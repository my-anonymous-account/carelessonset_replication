...
for(suffix_location in suffix_locations)
{
  ...
  for(prop_crls in c(0.0, prop_crlss_arr))
  {
    ...
    } else
    {
      ...
      load(paste0(dir_load, "/classification_", suffix_type, 
...
p_all <- 
  ...
  theme_bw() +
  ylim(0,1) +
  ggh4x::facet_grid2(rows = vars(measure), cols = vars(Setting), # separate x-axes
                     scales = "free_x", independent = "x", switch = "y") +
  labs(y = NULL) + 
  #facet_grid(rows = vars(measure), cols = vars(Setting), scales = "free") +
  xlab("Prevalence of partially careless respondents") + 
...
# save the plot
ggsave(filename = paste0("plot-simresults_", suffix_type, "_benchmarks", "_n=", n, "_classification_R=", R, ".pdf"),
       plot = p_all,
       path = dir_save,
       device = "pdf",
       width = 7.5,
