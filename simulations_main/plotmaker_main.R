...
P_crls_fnr <- 
  ...
  geom_line() +
  ylim(0,1) +
  facet_grid(rows = vars(type), cols = vars(onset), scales = "fixed", labeller = label_parsed) +
...
P_crls <- 
  ggplot(data = df_careless, 
         mapping = aes(x = prop_crls, y = value, color = method, linetype = sig_level, alpha = sig_level)) +
  geom_line() +
  facet_grid(rows = vars(measure), cols = vars(onset), scales = "free_y", labeller = label_parsed, switch = "y") +
  labs(y = NULL) + 
  scale_linetype_manual("Significance\nlevel", values = c("dashed", "solid", "twodash")) +
  scale_alpha_manual("Significance\nlevel", values = c(0.4, 1, 0.4)) + 
  scale_color_manual("CODERS", values = c("#619CFF", "#F8766D", "black")) +
...
P_att <- 
  ...
  geom_line() +
  ylim(0,1) + 
  facet_grid(cols = vars(onset), labeller = label_parsed) +
  scale_linetype_manual("Significance\nlevel", values = c("dashed", "solid", "twodash")) +
  scale_alpha_manual("Significance\nlevel", values = c(0.4, 1, 0.4)) + 
