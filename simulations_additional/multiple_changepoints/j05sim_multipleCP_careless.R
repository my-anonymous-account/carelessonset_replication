...
dir_save <- "simulations_additional/multiple_changepoints/results"
suffix_type <- "j05sim-multipleCP"
suffix_location <- "early" # don't change!
...
careless_end <- carelessness_end_fun(n = n)
...
for(a in seq_along(prop_crls_arr))
{
  ...
  for(r in seq_len(R)){
    ...
    data_random_tmp <- data_contamination(data_uncontam  = data[idx_random,,drop = FALSE],
                                          ...
                                          careless_end   = careless_end[idx_random], 
                                          type           = "random")
  ...
  save(out_CP_LS_SC, out_CP_LS, out_CP_SC,
       careless_start, idx_careful, idx_careless, idx_pattern, idx_random, idx_straight, idx_extreme, careless_end,
