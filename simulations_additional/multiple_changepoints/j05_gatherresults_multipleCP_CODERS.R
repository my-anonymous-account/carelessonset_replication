...
for(suffix_location in suffix_locations)
{
  ...
  for(prop_crls in prop_crls_arr)
  {
    ...
    onset2_true <- careless_end
    
    # set onset location of truly careful respondents to NA (expected by fun below)
    onset_true[idx_careful] <- onset2_true[idx_careful] <- NA_integer_ 
