...
plot_teststat <- function(x, d, alpha = c(0.05, 0.025, 0.01)) 
{
  ...
  p    <- length(x) + 1L
  khat <- which.max(x) + 1L # account for fact that indices run from 2 to p
  
  # get critical values
  Kn <- get_Kn(d = d, alpha = alpha)
  
  # prepare data frame and plot
  df <- data.frame(x = seq_len(p), y = c(0.0, x)) # account for fact that indices run from 2 to p
  gg <- ggplot(df, mapping = aes(x = x, y = y)) +
    theme_bw() +
    geom_line() +
    geom_vline(xintercept = khat, linetype = "solid", col = "blue", size = 0.7) +
    geom_hline(yintercept = Kn, linetype = "dashed", col = "grey50") + 
    ylab("Statistic") +
    xlab("Item index")
...
plot_dimension <- function(RE = NULL, LSP = NULL, time = NULL, onset = NULL, color = "blue", increase = TRUE)
{
  ...
  gg <- ggplot(df, mapping = aes(x = x, y = y)) +
    theme_bw() +
    geom_line() +
    facet_grid(rows = vars(statistic), scales = "free_y", switch = "y") +
    xlab("Item index") +
    # ylab("Score")
    ylab(NULL)
